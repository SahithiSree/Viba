...
public class HomeEditActivity extends AppCompatActivity {
    ...
    private Button btnpost, btnchoose, btnclear;
    ...
    private String homepostkey, currentId, username, userProfileimage, userRole, userCountry;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Edit Bulletin");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        Intent intent = getIntent();
        homepostkey = intent.getStringExtra("homepostkey");

        progressBar = findViewById(R.id.progress_edithomepost);
        progressDialog = new ProgressDialog(this);

        firebaseAuth = FirebaseAuth.getInstance();

        imageView = findViewById(R.id.adeditpostImage);
        circleImageView = findViewById(R.id.adeditproImage);
        btnclear =findViewById(R.id.cleareditHomeDescp);
        editText = findViewById(R.id.adeditPostInfo);
        btnpost = findViewById(R.id.btneditHomepost);
        btnchoose = findViewById(R.id.chooseeditHomeImage);
        currentId = firebaseAuth.getCurrentUser().getUid();

        postReference = FirebaseDatabase.getInstance().getReference("Homepost Info").child(homepostkey);
        ...
        postReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    ...
                    String userDescpstr = dataSnapshot.child("homepd").getValue().toString();
                    String userHomeImage = dataSnapshot.child("homepi").getValue().toString();

                    Picasso.get().load(userHomeImage).into(imageView);
                    editText.setText(userDescpstr);
        ...
        btnpost.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (imageUri == null && TextUtils.isEmpty(description)) {
                    Snackbar.make(v, "Edit something....", Snackbar.LENGTH_LONG)
    ...
    private void storageImage() {
        ...
        postReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                downloadUrl = dataSnapshot.child("homepi").getValue().toString();
            }
        ...
        adReference.child(currentId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    userRole = dataSnapshot.child("adrole").getValue().toString();
                    userCountry = dataSnapshot.child("adctry").getValue().toString();
                    ...
                    postMap.put("homeuid", homepostkey);

                    postReference.updateChildren(postMap)
                            .addOnCompleteListener(new OnCompleteListener() {
                                @Override
                                public void onComplete(@NonNull Task task) {
                                    if (task.isSuccessful()) {
                                        progressDialog.dismiss();
                                        Toast.makeText(HomeEditActivity.this, "Post updated successfully!", Toast.LENGTH_SHORT).show();
                                        startActivity(new Intent(HomeEditActivity.this, HomeActivity.class));
                                        finish();
                                    } else {
                                        progressDialog.dismiss();
                                        Toast.makeText(HomeEditActivity.this, "Error occurred! Try again", Toast.LENGTH_SHORT).show();
                                    }
                                }
                            });
                }
